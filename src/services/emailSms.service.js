  ...
  try {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
        <h2 style="color: #2c3e50; text-align: center;">Neev Construction App OTP</h2>
        ...
        <p style="text-align: center; color: #bdc3c7; font-size: 12px;">© ${new Date().getFullYear()} Neev Construction App</p>
      </div>
    `;

    const info = await transporter.sendMail({
      from: process.env.EMAIL_FROM || `"Neev Construction" <${process.env.SMTP_USER}>`,
      to: email,
    ...
    return { success: true, messageId: info.messageId, otp: otp };
  } catch (error) {
